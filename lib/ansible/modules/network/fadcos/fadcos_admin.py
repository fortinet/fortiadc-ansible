...
---
module: fadcos_admin
short_description: Manages FortiADC admin by RESTful API
description:
  - Manages admin on FortiADC devices including creating, updating, removing admin objects,
...
from ansible.module_utils.network.fadcos.fadcos import fadcos_argument_spec, get_err_msg, list_to_str, list_need_update, is_global_admin, is_vdom_enable, is_user_in_vdom
import json
...
def param_check(module, connection):
    ...
    if module.params['vdom'] and not is_global_admin(connection):
        if len(module.params['vdom']) > 1:
            ...
            err_msg = 'user is not global admin can not access the vdom ' + module.params['vdom'][0] + ' for new admin'
            res = False
