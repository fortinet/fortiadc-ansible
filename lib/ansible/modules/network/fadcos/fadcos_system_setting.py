...
from ansible.module_utils.network.fadcos.fadcos import fadcos_argument_spec, is_global_admin
import json


def get__sys_setting(module, connection):
    payload = {}
    code, response = connection.send_request('/api/system_global', payload, 'GET')

    return code, response['payload']

def update_sys_setting(payload, connection):

    code, response = connection.send_request('/api/system_global', payload, 'PUT')
